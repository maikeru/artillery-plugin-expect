...
function expectNotHasProperty(expectation, body, req, res, userContext) {
  ...
  if (userContext.vars.notHasProperty) {
    userContext.vars['hasProperty'] = userContext.vars['notHasProperty'];
  }
  expectation.hasProperty = expectation.notHasProperty;
  let result = expectHasProperty(expectation, body, req, res, userContext);
  const expectedProperty = template(expectation.notHasProperty, userContext);
  result.expected = expectedProperty;
  result.ok = !result.ok;
  result.type = 'notHasProperty';
  if (!result.ok) {
